...
export default function MRCCompleta({ projetoId, projeto, clienteNome, projetoNome, notificacoes, papel }) {
  ...
  const fasesDisponiveis = [...new Set(mrc.map(r => getFaseInfo(r, projeto?.num_fases, projeto?.f1_tem_teste === true).label))].sort()
  ...
  return (
    <div className="mrc-wrap">
      ...
      <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexShrink: 0, flexWrap: 'wrap', marginBottom: 6 }}>
        ...
        <button style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: 'rgba(34,197,94,0.1)', border: '1px solid rgba(34,197,94,0.3)', borderRadius: 999, padding: '5px 10px', fontSize: 10, fontWeight: 600, color: '#16A34A', cursor: 'pointer', fontFamily: 'inherit', marginLeft: 'auto' }} onClick={() => exportarMRCExcel(filtered, 'MRC_' + (clienteNome || 'Cliente').replace(/\s+/g, '_') + '_Completa_' + new Date().toISOString().slice(0,10), 'MRC Completa', clienteNome, projetoNome, projeto)}>Excel</button>
      </div>
