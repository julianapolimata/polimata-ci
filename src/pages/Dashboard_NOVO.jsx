...
import ModalRevisaoCriticidadeF3 from '../components/ModalRevisaoCriticidadeF3'
import {
...
function PorArea({ projeto, areasCalc, todosControles, loading, navigate, loadDados }) {
  ...
  const [revisaoF3Row, setRevisaoF3Row] = useState(null)
  ...
  return (
    <div style={PA.page}>
              ...
              <tr key={c.id||i} onMouseEnter={e => e.currentTarget.style.background='rgba(204,145,94,0.04)'} onMouseLeave={e => e.currentTarget.style.background=''}>
                ...
                <td style={{ ...tdS, textAlign: 'center', width: 70, minWidth: 70 }}>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 3, alignItems: 'center' }}>
                    ...
                    {c.status_workflow === 'em_analise' && canEdit && <button onClick={() => setRevisaoF3Row(c)} style={{ background: 'rgba(102,0,51,0.1)', border: '1px solid rgba(102,0,51,0.3)', borderRadius: 3, padding: '2px 10px', fontSize: 10, fontWeight: 600, color: '#660033', cursor: 'pointer', fontFamily: 'inherit', width: '100%' }}>Revisar Crit.</button>}
      ...
      {revisaoF3Row && <ModalRevisaoCriticidadeF3 row={revisaoF3Row} onClose={() => setRevisaoF3Row(null)} onSaved={() => { setRevisaoF3Row(null); if (projeto?.id) loadDados(projeto.id) }} projeto={projeto} />}
