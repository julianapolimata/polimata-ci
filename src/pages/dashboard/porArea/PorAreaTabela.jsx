          ...
          <table style={{ width: 'max-content', minWidth: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>{cfSorted.map((c, i) => (
              <tr key={c.id||i} onClick={() => setModalRow(c)} style={{ cursor: 'pointer', ...((c.status_risco === 'evitado' || c.status_risco === 'transferido') ? { opacity: 0.55, fontStyle: 'italic' } : {}) }} onMouseEnter={e => e.currentTarget.style.background='rgba(204,145,94,0.04)'} onMouseLeave={e => e.currentTarget.style.background=''}>
                ...
                <td style={{ ...tdS, width: 130, minWidth: 130, fontSize: 11, fontWeight: 500, textAlign: 'center' }}>{getFaseLabel(c, projeto?.num_fases, projeto?.f1_tem_teste === true)}{c.num_regressoes > 0 && <RegressaoBadge n={c.num_regressoes} />}</td>
                <td style={{ ...tdS, width: 110, minWidth: 110, textAlign: 'center' }}>{(() => { const st = getStatusComputado(c, projeto?.num_fases, projeto?.f1_tem_teste === true); const cfg = getStatusBadge(st, c); const clicavel = st === 'teste_pendente' && canEditControle(c); if (clicavel) return <button onClick={e => { e.stopPropagation(); setAtualizarFicha(true); setAtualizarRow(c) }} title="Clique para baixar a ficha" style={{ fontSize: 10, fontWeight: 700, color: cfg.color, background: cfg.bg, padding: '3px 10px', borderRadius: 999, textTransform: 'uppercase', letterSpacing: 0.4, border: '1px solid rgba(204,145,94,0.45)', cursor: 'pointer', fontFamily: 'inherit' }}>📄 {cfg.label}</button>; return <span style={{ fontSize: 10, fontWeight: 700, color: cfg.color, background: cfg.bg, padding: '3px 10px', borderRadius: 999, textTransform: 'uppercase', letterSpacing: 0.4 }}>{cfg.label}</span> })()}</td>
                {/* Colunas de fase */}
                    ...
                    {(() => {
                      ...
                      return (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 4, alignItems: 'center' }}>
                          ...
                          {getAlertas(c).filter(a => !['Resultado Pendente', 'Revisar', 'Ficha Pendente'].includes(a.label) || !primary).map((a, idx) => {
