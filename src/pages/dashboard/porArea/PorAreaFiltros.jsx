                ...
                <div ref={excelMenuRef} style={{ position: 'relative', display: 'inline-block' }}>
                  ...
                  {excelMenuAberto && (
                    <div style={{ position: 'absolute', right: 0, top: 'calc(100% + 6px)', background: 'white', border: '1px solid var(--lt-border)', borderRadius: 8, boxShadow: '0 8px 24px rgba(0,0,0,0.08)', minWidth: 220, zIndex: 100, overflow: 'hidden' }}>
                      <button
                        onClick={() => { setExcelMenuAberto(false); exportarMRCExcel(cf, `MRC_${(formatNomeEmpresa(projeto?.clientes?.nome_fantasia || projeto?.clientes?.nome) || 'Cliente').replace(/\s+/g, '_')}_${nome.replace(/\s+/g, '_')}_${new Date().toISOString().slice(0,10)}`, nome, formatNomeEmpresa(projeto?.clientes?.nome_fantasia || projeto?.clientes?.nome) || '', projeto?.nome || '', projeto) }}
