...
import { getStatusConfig, CONFIG_CONCLUIDO, getProximaAcao } from '../../lib/statusWorkflow'
...
export default function PorArea({ projeto, areasCalc, todosControles, loading, navigate, loadDados }) {
  ...
  const [reabrirRow, setReabrirRow] = useState(null) // { c, modo } — reabrir controle inativo
  ...
  if (!area) return <div style={{ background: 'var(--lt-bg)', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 12, fontFamily: "'Montserrat', sans-serif" }}><div style={{ color: 'var(--lt-text3)' }}>Área não encontrada.</div><button onClick={() => navigate('/ci')} style={{ marginTop: 12, padding: '6px 16px', borderRadius: 999, border: '1px solid var(--lt-border)', background: 'var(--lt-card)', color: 'var(--lt-text)', cursor: 'pointer', fontFamily: 'inherit', fontSize: 11 }}>← Voltar</button></div>
  ...
  function renderFaseCell(c, idx) {
    ...
    if (['teste_pendente', 'em_analise', 'em_revisao'].includes(c.status_workflow) && KEY_POR_FASE[getFaseInfo(c, projeto?.num_fases, projeto?.f1_tem_teste === true).codigo] === key && !fezEtapa(c[key])) {
  ...
  const ctx = {
    ...
    setModalRow, setReabrirRow, setReavaliarRow, setRowCriticidade, setRowRegistrarResultado, setRowRevisar, setSimularPerfil, setSortCol,
  ...
  function getModalActions(c) {
    ...
    if (ehInativo) {
      ...
      if (pend) return canRevisar ? { primary: { label: '⟳ Decidir Reabertura', color: '#7C3AED', bg: 'rgba(124,58,237,0.10)', border: 'rgba(124,58,237,0.30)', onClick: () => { setReabrirRow({ c, modo: 'decidir' }); setModalRow(null) } } } : { bloqueado: true }
      return canEdit ? { primary: { label: '⟳ Reabrir', color: '#0F766E', bg: 'rgba(20,184,166,0.10)', border: 'rgba(20,184,166,0.30)', onClick: () => { setReabrirRow({ c, modo: 'solicitar' }); setModalRow(null) } } } : { bloqueado: true }
    }
    ...
    if (podeEditarEste && st === 'teste_pendente') return { primary: { label: '📄 Baixar Ficha', color: 'var(--copper-text)', bg: 'rgba(204,145,94,0.12)', border: 'rgba(204,145,94,0.45)', onClick: () => { setAtualizarFicha(true); setAtualizarRow(c); setModalRow(null) } }, secondary: { label: '✏ Editar', onClick: () => { setAtualizarRow(c); setModalRow(null) } } }
  ...
  return (
    <div style={PA.page}>
      ...
      {rowRegistrarResultado && <ModalRegistrarResultado row={rowRegistrarResultado} projeto={projeto} onClose={() => setRowRegistrarResultado(null)} onSaved={() => { setRowRegistrarResultado(null); if (projeto?.id) loadDados(projeto.id) }} responsaveis={[]} />}
      ...
      {reabrirRow && <ModalReabrir row={reabrirRow.c} perfil={perfil} modo={reabrirRow.modo} onClose={() => setReabrirRow(null)} onSaved={() => { setReabrirRow(null); if (projeto?.id) loadDados(projeto.id) }} />}
