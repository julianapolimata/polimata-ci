...
export default function Dashboard() {
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden' }}>
      ...
      <main style={{ flex: 1, overflowY: 'auto', background: 'var(--bg0)', position: 'relative' }}>
        {/* Notificações — canto superior direito */}
        <div style={{ position: 'sticky', top: 0, zIndex: 100, display: 'flex', justifyContent: 'flex-end', padding: '8px 16px 0' }}>
          <NotificacoesPanel />
        </div>
        <Routes>
                  ...
                  {isCliente ? (
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 3, alignItems: 'center' }}>
                      ...
                      {(() => {
                        ...
                        const scCfg = {
                          'Não Iniciado': { color: 'rgba(243,238,228,0.4)', bg: 'rgba(255,255,255,0.04)' },
                          'Em Análise': { color: '#CC915E', bg: 'rgba(204,145,94,0.12)' },
                          'Concluído': { color: '#22C55E', bg: 'rgba(34,197,94,0.12)' },
                        }[sc]
                        return <span style={{ fontSize: 8, fontWeight: 700, color: scCfg.color, background: scCfg.bg, padding: '2px 8px', borderRadius: 2, textTransform: 'uppercase', letterSpacing: 0.5 }}>{sc}</span>
                      })()}
                    </div>
                  ) : (
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 3, alignItems: 'center' }}>
                      <button onClick={() => setModalRow(c)} style={{ background: 'rgba(243,238,228,0.06)', border: '1px solid rgba(243,238,228,0.1)', borderRadius: 3, padding: '2px 10px', fontSize: 10, fontWeight: 600, color: 'rgba(243,238,228,0.6)', cursor: 'pointer', fontFamily: 'inherit', width: '100%' }}>Ver</button>
                      {canEdit && !['em_revisao','aprovado'].includes(c.status_workflow) && <button onClick={() => setAtualizarRow(c)} style={{ background: 'rgba(204,145,94,0.1)', border: '1px solid rgba(204,145,94,0.3)', borderRadius: 3, padding: '2px 10px', fontSize: 10, fontWeight: 600, color: '#CC915E', cursor: 'pointer', fontFamily: 'inherit', width: '100%' }}>Atualizar</button>}
                      {canEdit && (c.status_workflow === 'ficha_gerada' || c.status_workflow === 'reprovado') && <button onClick={() => setRowRegistrarResultado(c)} style={{ background: 'rgba(204,145,94,0.1)', border: '1px solid rgba(204,145,94,0.3)', borderRadius: 3, padding: '2px 10px', fontSize: 9, fontWeight: 600, color: '#CC915E', cursor: 'pointer', fontFamily: 'inherit', width: '100%' }}>{c.status_workflow === 'reprovado' ? '↩ Editar' : 'Resultado'}</button>}
                      {isAdmin && c.status_workflow === 'em_revisao' && <button onClick={() => setRowRevisar(c)} style={{ background: 'rgba(59,130,246,0.12)', border: '1px solid rgba(59,130,246,0.3)', borderRadius: 3, padding: '2px 10px', fontSize: 9, fontWeight: 700, color: '#60A5FA', cursor: 'pointer', fontFamily: 'inherit', width: '100%' }}>🔍 Revisar</button>}
                      {/* Badges de status */}
                      ...
                      {c.status_workflow === 'em_revisao' && <span style={{ fontSize: 8, fontWeight: 700, color: '#60A5FA', background: 'rgba(59,130,246,0.12)', padding: '1px 6px', borderRadius: 2, textTransform: 'uppercase', letterSpacing: 0.5 }}>Em Revisão</span>}
                      {c.status_workflow === 'aprovado' && <span style={{ fontSize: 8, fontWeight: 700, color: '#22C55E', background: 'rgba(34,197,94,0.12)', padding: '1px 6px', borderRadius: 2, textTransform: 'uppercase', letterSpacing: 0.5 }}>Aprovado</span>}
                      {c.status_workflow === 'reprovado' && <span style={{ fontSize: 8, fontWeight: 700, color: '#EF4444', background: 'rgba(239,68,68,0.12)', padding: '1px 6px', borderRadius: 2, textTransform: 'uppercase', letterSpacing: 0.5 }}>Reprovado</span>}
                    </div>
                  )}
                </td>
